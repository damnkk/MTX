...
int SceneGraph::addNode(int parent, int level) {
  ...
  //set node name
  uint32_t nameID = m_nodeNames.size();
...
void SceneGraph::collectNodesToDelete(int node, std::vector<uint32_t>& nodes) {
...
void shiftMapIndices(std::unordered_map<uint32_t, uint32_t>& items, std::vector<int>& indices) {
...
void SceneGraph::deleteSceneNodes(const std::vector<uint32_t>& nodesToDelete) {
  ...
  //recording array
  std::vector<uint32_t> nodes(m_nodeHierarchy.size());
  ...
  for (uint32_t i = 0; i < nodes.size(); ++i) { newIndices[nodes[i]] = i; }
